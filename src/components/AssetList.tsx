...
const AssetList: React.FC<AssetListProps> = ({ assets, onRequestInfo }) => {
  ...
  const getAssetTypeLabel = (type: AssetType): string => {
    const translations: Record<AssetType, string> = {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        <h2 className="text-2xl font-semibold text-estate-slate">Listado de Activos</h2>
      ...
      {showFilters && (
        <div className="bg-estate-offwhite p-4 rounded-md border border-estate-lightgrey">
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-medium text-estate-slate">Filtros</h3>
      ...
      {filteredAssets.length === 0 ? (
        <div className="text-center py-10 bg-estate-offwhite rounded-md">
          <p className="text-estate-steel">No se encontraron activos que coincidan con sus criterios.</p>
        </div>
      ) : (
